...
def _candidate_forms_from_lemma(lower: str) -> List[Tuple[str, str]]:
    ...
    if entry is not None:
        simple, gloss = entry
        variants.append((simple, gloss))
    if lower.endswith("s") and len(lower) > 3:
        singular = lower[:-1]
        entry = SIMPLE_WORDS.get(singular)
        if entry is not None:
            simple, gloss = entry
            variants.append((_pluralize_simple(simple), gloss))
    if lower.endswith("ed") and len(lower) > 4:
        for base in {lower[:-2], lower[:-1]}:
            entry = SIMPLE_WORDS.get(base)
            if entry is not None:
                simple, gloss = entry
                variants.append((_past_simple(simple), gloss))
    if lower.endswith("ing") and len(lower) > 5:
        for base in {lower[:-3], lower[:-3] + "e"}:
            entry = SIMPLE_WORDS.get(base)
            if entry is not None:
                simple, gloss = entry
                variants.append((_ing_simple(simple), gloss))
    seen = set()
...
def conservative_lexical_simplify(
    ...
    use_structural_simplification: bool = True,
) -> Tuple[str, List[Dict[str, str]], List[str]]:
    working_text = structural_sentence_simplify(text) if use_structural_simplification else text
    ...
    for sentence in split_sentences(working_text):
        ...
        for idx, token in enumerate(tokens):
            ...
            if decision is None:
                if use_glossary_fallback and lower not in replaced_words:
                    entry = SIMPLE_WORDS.get(lower)
                    if entry is not None and _is_hard_word(lower, frequency_rank, max_word_length, min_word_frequency_rank):
                        glossary.append(entry[1])
                continue

            if remaining_budget <= 0 or float(decision["confidence"]) < min_replacement_confidence:
                if use_glossary_fallback:
                    glossary.append(str(decision["gloss"]))
                continue
    ...
    if use_glossary_fallback:
        for token in tokenize(working_text):
            lower = token.lower()
            ...
            if entry is None:
                continue
            if _is_hard_word(lower, frequency_rank, max_word_length, min_word_frequency_rank):
                glossary.append(entry[1])
            if len(dict.fromkeys(glossary)) >= max_glossary_items:
                break

    glossary = list(dict.fromkeys(glossary)) if use_glossary_fallback else []
    return simplified, replacements, glossary[:max_glossary_items]
